...
public class dbp_get {
    ...
    static public void main(String[] args) throws Exception {
        ...
        // 出力ファイル指定
        File fileOUT = new File("output_dis/dbp.ttl");
        // 出力用のファイルのWriterの設定
        ...
        try (QueryExecution qexecWikidata = QueryExecutionHTTP.create()
            ...
            bw.write ("@prefix owl: <http://www.w3.org/2002/07/owl#> .\n"
            		...
            		+ "@prefix dis_pc: <https://hozo.jp/dis/prop/wikiLink/category/> .\n"
            		+ "@prefix dis_pwd: <https://hozo.jp/dis/prop/wikiLink/wikidata/> .\n");
            ...
            while (rsWikidata.hasNext()) {
                ...
                String dbpediaQueryStr = "PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#> " +
                                         ...
                                         "SELECT ?wikiPage2 ?disLabel ?symLabel ?symID ?symID2 ?disID " +
                                         "WHERE { " +
                                         "<" + wikipedia + "> dbo:wikiPageWikiLink ?wikiPage . " +
                                         "<" + wikipedia + "> dbo:wikiPageWikiLink ?wikiPage2 . " +
                                         "<" + wikipedia + "> rdfs:label ?disLabel . " +
                                         "?wikiPage2 dct:subject|dct:subject/skos:broader dbpedia-ja:Category:症状と徴候 ."+
                                         "?wikiPage2 rdfs:label ?symLabel . "+
                                         "?symID owl:sameAs ?wikiPage  ."+
                                         "?symID2 owl:sameAs ?wikiPage2  ."+
             ...
             // DBpediaのクエリの実行
                ...
                try (QueryExecution qexec = QueryExecutionHTTP.create()
                	...
                	while (rsDBpedia.hasNext()) {
                        ...
                        Resource wikiPage2 = qs.getResource("wikiPage2");
                        Resource symID = qs.getResource("symID");
                        Resource symID2 = qs.getResource("symID2");
                        ...
                        bw.write("<" +wikipedia+"> owl:sameAs <" + disName + "> .\n"
                        		+ "<" +wikiPage2 + "> owl:sameAs <" + symName + "> .\n"
                        		+ "<" +wikipedia+"> dis_pc:sym " + "<" +wikiPage2 + "> .\n" 
                        		+ "<" +wikipedia+"> rdfs:label \"" + disLabel + "\"@ja .\n" 
                        		+ "<" +wikiPage2 + "> rdfs:label \"" + symLabel + "\"@ja .\n") ;
