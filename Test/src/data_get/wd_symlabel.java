...
public class wd_symlabel {
    ...
    static public void main(String[] args) throws Exception {
        ...
        // 出力ファイル指定
        File fileOUT = new File("output_dis/wd_symlabel.ttl");
        // 出力用のファイルのWriterの設定
        ...
        String wikidataQueryStr = "PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#> " +
                                  ...
                                  "SELECT DISTINCT ?sym ?symLabel "+
                                  "WHERE {" +
                                  "?dis wdt:P494 ?o ."  +       // ?dis（疾患）にP494のプロパティを持つもの
                                  "?dis wdt:P780 ?sym . "  +     // ?dis（疾患）にP780（症状）プロパティを持つもの
                                  "?sym rdfs:label ?symLabel ." +
                                  "FILTER (lang(?symLabel) = 'ja') "+
                                  "}"+
                                  "ORDER BY DESC(?symLabel)" ;     
